...
describe('Stake', () => {
    ...
    beforeEach(async () => {
        ...
        stake = blockchain.openContract(
            Stake.createFromConfig(
                {
                    lock_time: 1800,
                    not_addr: jettonMinter.address,
                    not_bytecode: codeWallet,
                    admin_addr: owner.address,
                },
                code,
            ),
        );
    ...
    it('should deploy and stake TONNEL', async () => {
        ...
        blockchain.now = Math.round(timestamp / 1000) + 1800;

        console.log(await stake.getUserState(sender.getSender().address));
        const withdrawTonnelResult = await stake.sendWithdrawNot(sender.getSender(), {
            queryID: 0,
            value: toNano('0.02'),
        ...
        expect(await jettonWalletStakeContract.getBalance()).toEqual(toNano(40));
        expect(await jettonWalletStaker.getBalance()).toEqual(toNano(60));
        const withdrawTonnelResult2 = await stake.sendWithdrawTON(sender.getSender(), {
            queryID: 0,
            value: toNano('0.02'),
